...
const ListCredit = ({ credits, setCredits, creditInfo }: ListCreditProps) => {
  const grades = creditInfo.passGrade
    .concat(creditInfo.failGrade)
    .concat(creditInfo.unknownGrade);
  ...
  return (
    <>
      <Button className="mb-2" onClick={() => exportCredits()}>
        単位一覧のエクスポート
      </Button>
        ...
        <tbody>
          <AddCredit
            addCredit={(credit: Credit) =>
              setCredits({ ...credits, credits: [credit, ...credits.credits] })
            }
            creditInfo={creditInfo}
          />
