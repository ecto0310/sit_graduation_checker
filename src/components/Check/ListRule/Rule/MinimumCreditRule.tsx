...
export const CalcMinimumCreditRule = (
  rule: MinimumCreditRule,
  credits: Credit[],
  passGrade: string[],
): [Result, number, Credit[]] => {
  const passCredits = credits.filter((credits) =>
    passGrade.includes(credits.grade),
  );
  ...
  const sumCredit = targetCredits.reduce(
    (sum, targetCredit) => sum + targetCredit.count,
    0,
  );
...
const MinimumCreditRule = ({
  rule,
  credits,
  passGrade,
}: MinimumCreditRuleProps) => {
  const [result, sumCredit, targetCredits] = CalcMinimumCreditRule(
    rule,
    credits,
    passGrade,
  );
  const nonPassrequiredSubjects = rule.requiredSubjects?.filter(
    (requiredSubject) =>
    ...
    <>
      <tr>
        ...
        <td>
          ...
          {nonPassrequiredSubjects?.map((requiredSubject, index) => (
