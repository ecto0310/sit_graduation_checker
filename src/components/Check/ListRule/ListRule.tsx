...
import MinimumCreditRule, {
  CalcMinimumCreditRule,
} from "./Rule/MinimumCreditRule";
...
const ListRule = ({ credits, rules }: ListRuleProps) => {
  ...
  const filter = (): Credit[] => {
    ...
    let sortKey = rules.creditInfo.passGrade
      .concat(rules.creditInfo.failGrade)
      .concat(rules.creditInfo.unknownGrade);
  ...
  const results = (): Result[] => {
    return rules.rules.map((rule) => {
      if (rule.type == "minimumCredit") {
        ...
        const [result] = CalcMinimumGPARule(
          rule,
          filter(),
          rules.creditInfo.gradePoint,
          isSchedule,
        );
        return result;
      }
  ...
  const result = (): Result => {
    ...
    return results_cache.every((result) => result == "pass")
      ? "pass"
      : results_cache.some((result) => result == "fail")
        ? "fail"
        : "unknown";
  };
  ...
  return (
    <>
      <h3>
        判定結果: <ResultMark result={result()} />
      </h3>
      <Form.Check
        className="m-1"
        type="switch"
        label="取得予定単位を含める"
        onClick={(e) => setIsSchedule(!isSchedule)}
      />
            ...
            if (rule.type == "minimumCredit") {
              ...
              return (
                <MinimumGPARule
                  ...
                  rule={rule}
                  credits={filter()}
                  gradePoint={rules.creditInfo.gradePoint}
